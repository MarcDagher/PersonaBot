...
def extract_kg_elements(text, topic, max_retries=3, delay=5):
    llm = ChatGroq(temperature=0, model_name="llama-3.1-8b-versatile")
...
def create_knowledge_graph(entities: List[Entity], relationships: List[RelationshipModel], neo4j_uri: str, neo4j_user: str, neo4j_password: str):
    ...
    print(f"Created or updated {len(nodes)} unique nodes and {relationships_created} unique relationships in Neo4j.")
...
def construction():
    ...
    folder_path = r'C:\Users\Nagham\OneDrive\Desktop\Llama3.1 Project\knowledge_docs'
