...
from typing import Dict, TypedDict, Annotated # to construct the agent's state
...
def test_agent():
    ...
    # Test input
    input_message = "What are some personality traits associated with social occupations?"
    ...
    # Configuration for the StateGraph
    config = {
        "configurable": {
            "thread_id": "1",
            "checkpoint_ns": "test",
            "checkpoint_id": "test_run"
        }
    }
    ...
    try:
        ...
        logger.exception(f"An error occurred during agent execution: {str(e)}")
    ...
    if final_state:
        ...
        logger.info(str(final_state))
        logger.info("-------------------")
        logger.info("Final extracted data:")
        for data in final_state.get('extracted_data', []):
            logger.info(data)
        logger.info("-------------------")
        logger.info(f"Number of queries made: {final_state.get('num_queries_made', 0)}")
    else:
        logger.warning("No final state was reached. The agent might not have completed its task.")
        logger.info("Last known state:")
