...
class Agent:
    ...
    def extract_data(self, state: AgentState):
        ...
        # Scan output
        safe_output = self.scan_output(extracted_data.content, prompt[-1].content)
        if safe_output is None:
            return {"extracted_data": ["I apologize, but I can't provide that response due to security concerns."]}
        
        return {"extracted_data": [safe_output]}
...
def test_agent():
    ...
    # Initialize model
    model = ChatGroq(temperature=0.7, groq_api_key=os.getenv('GROQ_API_KEY'), model_name="llama-3.1-70b-versatile")
    ...
    # Test message
    test_message = HumanMessage(content="I love to sleep")

    # Run the agent
    config = {"configurable": {"thread_id": "test"}}
    for event in agent.graph.stream({"conversation": [test_message]}, config, stream_mode="values"):
        print("AI Response:", event["conversation"][-1].content)
        print("=" * 50)

    # Get final state
    final_state = agent.graph.get_state(config=config).values
    
    print("\nFinal State:")
    print(f"Number of queries made: {final_state['num_queries_made']}")
    print(f"Cypher code and query outputs: {final_state['cypher_code_and_query_outputs']}")
